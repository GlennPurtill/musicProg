...
export class PatternItemComponent implements OnInit {
  ...
  ngOnInit() {
    this.length = 8;
    this.title = 'PATTERN A';
    this.structure = ['C4', 'D4', 'E4', 'F4', 'G4', 'A4', 'B4'];
    this.id = 1;
    this.pattern = new Pattern(this.id, this.length, this.title, this.structure)
   // this.numberTest = this.patternService.getTest()
  }
